      ...
      <div className='flex flex-col gap-6'>
        ...
        {filteredEstimates.length === 0 ? (
          <div className='p-12 rounded-xl bg-zinc-50 border border-zinc-200 border-dashed flex flex-col items-center justify-center gap-2 text-center'>
            ...
            <p className='text-zinc-500 text-sm max-w-sm'>{filter === 'all' ? 'No hay presupuestos en proceso de negociación actualmente.' : 'No hay presupuestos con el estado seleccionado.'}</p>
            {filter !== 'all' && <StatusFilter options={[]} allLabel='Ver todos' />}
          </div>
        ) : (
