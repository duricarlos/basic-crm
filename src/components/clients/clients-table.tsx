    ...
    <div className='space-y-4'>
      <div className='flex flex-col md:flex-row md:items-center justify-between gap-4'>
        ...
        <div className='flex flex-wrap gap-2'>
          <Button variant='outline' size='sm' onClick={() => setStatusFilter('all')} className={`rounded-full border transition-all ${statusFilter === 'all' ? 'bg-zinc-900 text-white hover:bg-zinc-800 border-zinc-900' : 'bg-white text-zinc-500 hover:text-zinc-900'}`}>
            Todos
          </Button>
          {filterOptions.map((opt) => (
            <Button key={opt.value} variant='outline' size='sm' onClick={() => setStatusFilter(opt.value)} className={`rounded-full border transition-all ${statusFilter === opt.value ? 'bg-zinc-900 text-white hover:bg-zinc-800 border-zinc-900' : 'bg-white text-zinc-500 hover:text-zinc-900'}`}>
              {opt.label}
              <span className={`ml-2 text-xs font-medium px-1.5 py-0.5 rounded-full ${statusFilter === opt.value ? 'bg-zinc-700 text-white' : 'bg-zinc-100 text-zinc-600'}`}>{opt.count}</span>
            </Button>
          ))}
        </div>
        ...
        <div className='relative md:w-auto w-full'>
          ...
          <Input type='search' placeholder='Buscar cliente...' className='pl-9 w-full md:w-[250px] bg-white' value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
        </div>
      </div>
